...
Every component of ThisIsMyPC, the Avalonia UI, the Session 0 service, the kernel driver, and all security implementation logic, is licensed under the GNU General Public License v2, version 2 only. The "or any later version" option is not offered: the repo includes code derived from ExplorerPatcher and OpenRGB, and both are GPLv2 without that clause, so the combined work cannot be relicensed under a later version. This is not a provisional stance or a placeholder until the project matures. It is a permanent architectural commitment, and the reasoning behind it is rooted in security engineering, not ideology.
...
1. **Audit first.** Before assuming incompatibility, we verify whether our GPLv2 dependencies actually carry the "or later" clause. Audit outcome as of 2026-09-01: ExplorerPatcher and OpenRGB do not, so the project is v2-only and this path is closed for code derived from them. Many community-driven open-source projects adopt it by default following the FSF's recommendation. If the clause is already present, the combined work can be distributed under GPLv3, and Apache 2.0 compatibility is resolved without any action from anyone.
