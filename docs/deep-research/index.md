...
| Document | Purpose |
|---|---|
| [context-menu-gap-closure.md](./context-menu-gap-closure.md) | Deductive gap closure analysis resolving 5 open questions from the context menu research corpus: per-handler Tier 1 promotion (confirmed negative), OneDrive single vs multi-handler architecture (single FileSyncEx), content-inspecting handler scan depth (top-level only), NVIDIA App CLSID verification status (likely hallucinated), drag-drop modern handler participation (permanently legacy-only). |
