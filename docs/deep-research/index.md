...
AI deep-research output (Gemini, converted from PDF) used for architectural and implementation decisions. Read SUMMARY.md; the full documents are the cited sources.

## Master Summary

| Document | Lines | Purpose |
|---|---|---|
| [SUMMARY.md](./SUMMARY.md) | ~600 | Unified synthesis of the nine research documents below (190 pages) into a single reference covering the Windows 11 trust model, enforcement drivers, IPC/kernel attack surfaces, bytecode interpreter security, supply chain threats, context menu architecture (contribution taxonomy, Explorer pipeline, ghost handlers, surface inheritance, "New" submenu, multi-selection logic, AppModel State Repository, diagnostic probe requirements, DragDropHandler invocation path, OneDrive cloud provider injection, content-aware handlers, Tier 1 promotion hack, NVIDIA dual-registration, eight-tier handler taxonomy), configuration surface mapping, and user-mode runtime integrity (CFG/ACG/CIG). |
