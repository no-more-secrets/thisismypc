...
With the bundled OpenRGB ([release/companions.md](../release/companions.md)) `OpenRgbBundled` is true. The bundled copy counts as installed. When no OpenRGB runs, the action is StartService and the page starts the headless server as it opens. A user's own OpenRGB running without its SDK server keeps the Open action, so no second server is started beside it. An Available Lighting tab shows no button; the device cards are the controls.
...
- Lighting device controls (2026-09-10): `Core/Hardware/Lighting/` holds the models, the controller-description parser and the update payloads (`LightingWire`, field order from RGBController.cpp, protocol version 4). It also holds the `IOpenRgbSession` and `IOpenRgbHost` contracts. `Interop.Win32/Hardware/OpenRgbSdkClient` is the TCP session; `BundledOpenRgbHost` runs the bundled server. `App/ViewModels/LightingControlsViewModel` renders one card per device: mode, brightness, speed, direction, random colors, one color per mode color or per zone, and Save where supported. Writes coalesce latest-wins. Every write asks the tab's live decision for WriteDevices first.
