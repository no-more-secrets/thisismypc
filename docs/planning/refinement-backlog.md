...
## Lighting device controls with bundled OpenRGB (2026-09-10)

- DONE: OpenRGB ships with the app ([release/companions.md](../release/companions.md)). `tools/get-openrgb-archive.ps1` hash-checks the pinned 1.0rc3.1 archive. `copy-bundled-companions.ps1` puts the headless subset under `companions\OpenRGB` in the release staging tree with a NOTICE. The signing wrapper and the reproducible comparison skip that folder. Inside a checkout the app runs the extracted cache copy.
- DONE: `BundledOpenRgbHost` starts the server headless on localhost with its own config folder, in a kill-on-close job. The policy asks for it only when no OpenRGB runs at all, so a user's own copy is never doubled. It waits for the port and for the device count to hold still. The app stops its own server on exit.
- DONE: SDK client and codec. `LightingWire` parses the controller description for protocol versions 0 to 6. It writes UPDATELEDS, UPDATEZONELEDS, UPDATESINGLELED, SETCUSTOMMODE, UPDATEMODE and SAVEMODE. `OpenRgbSdkClient` negotiates version 4 and matches replies by id. It skips the server's greetings and device-list notifications, and a vanished device ends the list instead of the session.
- DONE: Lighting page controls. One card per device: mode, brightness, speed, direction, random colors, the mode's colors or an All LEDs row plus one per zone, Save to device. Sliders and colors apply live with latest-wins coalescing. Every write re-checks the decision's WriteDevices. The Advanced override shows the cards greyed out and refuses writes.
- Verified: pure codec tests against a hand-built description. Rendered dark/light device cards over a scripted session record mode, brightness, zone, all-LED and save writes. The live diagnostic on Sam's desktop started the bundled server and listed the RTX 4080 (30 LEDs, ten modes) and a Glorious mouse over protocol 4. A live write painted the RTX 4080 red through Direct and restored Off. The real Lighting page started the service on open and listed both devices; edge geometry reads 25/17/10.
- Not verified: SMBus devices (PawnIO is not bundled). A signed release build with the companions folder; the signing exclusion and the comparison rule are new. The installed app's job-object cleanup. Inverted speed ranges on the GPU in the real UI.
- Open: saved lighting presets for this PC, PawnIO for motherboard and RAM lighting, elevation for devices that need it, protocol 6 features (per-zone modes).
...
- Open: the walkthrough times out on Privacy & Telemetry after the Hardware pages; unrelated to this batch, observed on 2026-09-09, not investigated. Next batches: Lighting device controls over the SDK, Monitoring's LibreHardwareMonitor backend, then Cooling presets and G-Helper model verification.
