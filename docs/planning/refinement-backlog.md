...
- DONE: a click's outcome never sits inside the card. `IUserFeedback` (`App/Services`) routes a result to a toast titled after the thing acted on and a failure to the window status line, which now carries its full text as a tooltip. The status line itself used to add a second row to the apply bar (Sam's screenshot: "Explorer restarted successfully" under the buttons); it now shares the button row, one trimmed line in the space after the pending count, so the bar never changes height either. A card that keeps its failure marks it with a red `issue-marker` icon beside the button, tooltip carrying the text. Converted: Lighting Apply (an unapplied edit makes Apply the accent button), companion Install and Open, Display writes, the Owner Mode card and the card-level Turn on Owner Mode, the custom context menu form, the new power plan name (the box turns red), Save as Preset, Settings export and import, the History panel.
...
- DONE (2026-09-22): restart and reboot notices are sticky toasts (Warning, kept until closed or done, one at a time under a key; the refresh note fades like any result) instead of a bar docked above the apply bar. Restart Explorer is a permanent button in the Explorer page header, next to refresh, and turns accent while applied changes wait for it. `ToastStack.Show` gained `sticky` and `key`; sticky cards never auto-dismiss, never yield to newer cards, and push nothing out.
- Open: the Lighting tab's top status line and the scan error lines on Display, Environment, and Startup still appear at load time above the cards; they do not change after a click, so they stay. The Debug page's simulation bar is still docked (Debug builds only).
- DONE (2026-09-22): the six Diagnostic suites that still clicked module names by text (reload tabs, Explorer edge, edge geometry, hover audit, Fluent navigation, Autoruns live) go through `OpenModule`, since a module in a folded group has no visible text; the collapsed-sidebar icon check picks the visible chevron of a group header. The Autoruns live shot also found that the 2026-09-06 hardening had left the shell type icon and Authenticode services out of the app graph because the UI ran elevated then; the unelevated UI registers `FileIconService` and `AuthenticodeService` again, so the Startup & Services rows carry icons and "(Verified)" signers in production.
...
- DONE: `src/ThisIsMyPC.Lighting` ([lighting-controllers.md](../lighting-controllers.md)): transport interfaces (`IHidTransport`, `II2cBus`), a detector registry mirroring OpenRGB's (vendor, product, interface, usage page, usage; PCI ids plus address), `ILightingController`, and `NativeLightingBackend` (one cached pass, sessions shared over the controllers, every call off the UI thread under a per-controller lock). Pure and unit-tested against a scripted HID bus and a simulated ENE chip.
- DONE: Sinowealth mice (Glorious Model O / D, Everest GT-100): the two-collection detection with OpenRGB's remainder rule, ten modes, the 520-byte configuration report with R, B, G colors.
- DONE: ENE SMBus for ASUS graphics cards: the register protocol, firmware-name register layouts, zones from the config table, ten modes plus Double Fade where supported, random-color variants, Direct and effect color tables in blocks of three with a byte-wise fallback, manual Save to flash. `EneGpuDetectors.g.cs` (172 PCI ids) is generated by `tools/import-openrgb-pci-detectors.ps1` from OpenRGB's detector file; `tools/fetch-openrgb-controller.ps1` pulls a controller folder from GitLab for the next port.
- DONE: transports in Interop.Win32: `WindowsHidTransport` (setupapi + hid.dll, hidapi semantics) and `NvApiI2cBusProvider` (nvapi64.dll through `nvapi_QueryInterface`, `NV_I2C_INFO_V3`, port 1). `VendorNativeDependencyLoader` maps nvapi64.dll from System32 before CIG after checking the NVIDIA signature; absent or unverifiable is a logged skip, never fatal.
- DONE: policy and facts. `ObservedHardwareFacts.LightingDevices` comes from the backend through `HardwareFactsProvider` (a refresh rescans). Lighting has no companion: null is Unknown, empty is Unavailable with no button, devices give Available with WriteDevices. OpenRGB joined the likely interferers; serving devices makes it a Conflict. `StartService`, `OpenRgbBundled`, the bundled host, the companion manifest and scripts, and the signing and comparison exclusions are gone.
- Verified: 22 controller tests, the policy and facts tests, rendered device cards, the no-device and conflict pages. Live on Sam's desktop: detection in 328 ms found the Glorious Model O (firmware V103) over HID and the RTX 4080 Strix (AUMA0-E6K5-1110, 30 LEDs, subsystem 1043:88BF) over NvAPI at 0x67; a write painted both red and restored Static and Off. `NvApiUnderCigTests` mapped nvapi64.dll, enabled CIG in the test host, and still probed the GPU.
- Not verified: the installed NativeAOT app under CIG on a GPU machine (the test host proved the mechanism, not the shipped exe). Mode changes on the mouse beyond Static. Non-ASUS or AMD cards. Machines with more than one supported device of the same family.
- Open: PawnIO SMBus for motherboards and memory (needs the driver, administrator rights, and a bus provider), AMD GPUs (ADL), more HID families, saved lighting presets for this PC, per-LED painting.

## Hardware companion tabs (2026-09-09)

- DONE: Module-level detection (v1 plan section 5, batch 2). `HardwareFactsProvider` in Core starts from the shared `IHardwareDetectionService` snapshot. It adds what the tabs need and the inventory leaves unobserved. Every companion gets a confirmed observation, so an absent one reads NotInstalled and an install can be offered. Launch paths come from uninstall entries, known and winget-portable folders, the FanControl scheduled task, HKCU Run values and running processes. Process paths are read across the elevation boundary. Ownership is recorded for FanControl with a saved configuration, OpenRGB with devices, and Armoury Crate services. The OpenRGB SDK server is probed on localhost only while OpenRGB runs. The Display module supplies the internal-panel fact, read once. Every native call sits behind `IHardwareProbeEnvironment` in Interop.Win32. The companion rules and the OpenRGB codec are pure and unit-tested with a scripted machine and a scripted inventory.
- DONE: System Control, Lighting, Cooling and Monitoring tabs (`Modules.Hardware`, one shared page). Each tab renders the policy's decision: a status badge, the explanation, and the one companion button the policy granted. Install queues the winget action through Software's pending-actions queue. Open launches the companion as the desktop user. A closed Details expander carries the evidence and detection notes. Tabs stay in the sidebar whatever the decision says. Pages open from the cached snapshot; a cache older than 30 seconds is re-checked behind the page, and Refresh forces a fresh pass.
- DONE: Settings > Advanced "Show all hardware controls" switch. It only sets `ControlsVisible`. Every action re-checks the decision's permitted operations, and the module test asserts the operations do not change with it.
- DONE: catalog entries for FanControl (`Rem0o.FanControl`), G-Helper (`seerge.g-helper`) and LibreHardwareMonitor. The winget ids were confirmed with `winget search` on 2026-09-09.
