...
public class InstallerShotTests
{
    ...
    [AvaloniaFact]
    public void KeyboardTraversal_IncludesFieldsAndChoicesAndSkipsDisabledActions()
    {
        var viewModel = new InstallerViewModel(new FakeEngine(), License, installed: null, existing: null)
        {
        ...
        using var window = new InstallerWindow(viewModel);
        window.HandleKey(NativeMethods.VK_TAB, reverse: true);
        Assert.Equal(InstallerWindow.HitTarget.Cancel, window.KeyboardFocus);
        window.HandleKey(NativeMethods.VK_TAB, reverse: true);
        Assert.Equal(InstallerWindow.HitTarget.Back, window.KeyboardFocus);
        window.HandleKey(NativeMethods.VK_TAB, reverse: true);
        Assert.Equal(InstallerWindow.HitTarget.LicenseAccepted, window.KeyboardFocus);
        Save(viewModel, "license-keyboard-focus", keyboardFocus: window.KeyboardFocus);
        window.HandleKey(NativeMethods.VK_SPACE);
        Assert.True(viewModel.LicenseAccepted);
        window.HandleKey(NativeMethods.VK_TAB, reverse: true);
        Assert.Equal(InstallerWindow.HitTarget.LicenseEdit, window.KeyboardFocus);

        viewModel.Step = InstallStep.Options;
        var seen = new HashSet<InstallerWindow.HitTarget>();
        for (var index = 0; index < 20; index++)
        {
            window.HandleKey(NativeMethods.VK_TAB);
            seen.Add(window.KeyboardFocus);
        }
        Assert.Contains(InstallerWindow.HitTarget.FolderEdit, seen);
        Assert.Contains(InstallerWindow.HitTarget.Browse, seen);
        Assert.Contains(InstallerWindow.HitTarget.StartMenu, seen);
        Assert.Contains(InstallerWindow.HitTarget.Desktop, seen);
        Assert.Contains(InstallerWindow.HitTarget.StartWithWindows, seen);
        Assert.Contains(InstallerWindow.HitTarget.CheckForUpdates, seen);
    }
    ...
    public async Task NativeMessageLoop_KeyboardAcceptsLicenseAndCompletesFakeInstall()
    {
        ...
        try
        {
            await WaitForAsync(() => window.WindowHandle != nint.Zero || failure is not null);
            Assert.Null(failure);
        ...
        finally
        {
            if (window.WindowHandle != nint.Zero)
                _ = NativeMethods.PostMessage(window.WindowHandle, NativeMethods.WM_CLOSE, 0, nint.Zero);
            Assert.True(thread.Join(TimeSpan.FromSeconds(5)), "Installer message thread did not stop.");
        }
        Assert.Null(failure);
    }
