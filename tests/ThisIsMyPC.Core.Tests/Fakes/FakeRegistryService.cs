...
/// Read-focused port of the Shell.Tests fake; records ReadString calls so tests
/// can assert session caching (detection happens exactly once).
/// </summary>
public sealed class FakeRegistryService : IRegistryService
{
    ...
    {
        ReadStringCallCount++;
        return ReadStringCore(keyPath, valueName);
    }

    public OperationResult<string> ReadExpandString(string keyPath, string valueName)
        => ReadStringCore(keyPath, valueName);

    private OperationResult<string> ReadStringCore(string keyPath, string valueName)
    {
