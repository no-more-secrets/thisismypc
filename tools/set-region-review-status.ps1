...
param(
    ...
    [Parameter(Mandatory)][ValidateRange(1, [int]::MaxValue)][int]$FigureNumber,
...
try {
    ...
    $figure = @($all | Where-Object { $_.number -eq $FigureNumber })
    if ($figure.Count -ne 1) { throw 'Figure was not found or its number is ambiguous.' }
    $figure = $figure[0]
    if ($null -ne $lease) {
        ...
        if (-not $already) {
            ...
            if ($Status -eq 'resolved') { $record.resolvedFigures += $figure } else { $record.figures += $figure }
        ...
        [pscustomobject]@{ applied = $true; status = $Status; sessionId = $SessionId; figureNumber = $FigureNumber } | ConvertTo-Json
        return
    }
